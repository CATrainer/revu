...
def upgrade():
    ...
    # Add session metadata columns only if they don't exist
    ...
    # Create session_tags join table only if it doesn't exist
    ...
    # Create session_shares table only if it doesn't exist
    if 'session_shares' not in existing_tables:
        op.create_table(
            'session_shares',
            sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, default=uuid.uuid4),
            sa.Column('session_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('ai_chat_sessions.id', ondelete='CASCADE'), nullable=False),
    ...
    # Create session_collaborators table only if it doesn't exist
    if 'session_collaborators' not in existing_tables:
        op.create_table(
    ...
    # Create message_comments table only if it doesn't exist
    if 'message_comments' not in existing_tables:
        op.create_table(
            'message_comments',
            sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, default=uuid.uuid4),
            sa.Column('message_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('ai_chat_messages.id', ondelete='CASCADE'), nullable=False),
    ...
    # Create full-text search indexes with IF NOT EXISTS
    op.execute("""
