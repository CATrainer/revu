...
# =============================================================================
# Notification Preferences Model (shared by creators and agency users)
# =============================================================================
...
class NotificationPreference(Base):
    ...
    # Email delivery mode: 'instant' or 'daily_digest'
    email_frequency = Column(String(20), nullable=False, default='instant')
    
    # Daily digest settings
    digest_hour = Column(Integer, nullable=False, default=9, comment="Hour to send digest (0-23 UTC)")
    last_digest_sent_at = Column(DateTime(timezone=True), nullable=True)
    
    # Per-type settings
    # Format: {"engagement_spike": {"in_app": true, "email": true}, ...}
    type_settings = Column(JSONB, nullable=False, default=dict)
    
    # Muted entities (don't notify about these specific items)
    # Format: [{"type": "campaign", "id": "uuid"}, ...]
    muted_entities = Column(JSONB, nullable=False, default=list)
    ...
    def is_type_enabled(self, notification_type: str, channel: str = 'in_app') -> bool:
        ...
        Returns:
            bool: Whether notifications of this type should be sent via this channel
        """
    ...
    def is_entity_muted(self, entity_type: str, entity_id: str) -> bool:
        """Check if a specific entity is muted."""
        for muted in (self.muted_entities or []):
            if muted.get('type') == entity_type and str(muted.get('id')) == str(entity_id):
                return True
        return False
