...
export default function ShareClient() {
  ...
  useEffect(() => {
    // In a real app, validate token server-side.
  }, []);
      ...
      <Card className="card-background border-[var(--border)]">
        ...
        <CardContent>
          <div className="space-y-3">
            {payload.title && (
              <div className="text-primary-dark font-medium">{payload.title}</div>
            )}
            <div className="text-secondary-dark text-sm">
              This link opens a read-only view in the app.
            </div>
            <div className="flex gap-2">
              <Button className="button-primary" onClick={() => router.replace(payload.route)}>
                Open
              </Button>
              <Button
                variant="outline"
