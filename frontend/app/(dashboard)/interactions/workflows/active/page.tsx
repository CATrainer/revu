  ...
  return (
    <div className="space-y-6 px-4 md:px-0">{/* Mobile padding */}
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">{/* Stack on mobile */}
        ...
        <Link href="/interactions/workflows" className="text-sm text-brand-primary hover:underline">Back to Workflows</Link>
      </div>
      ...
      <Card className="card-background border-[var(--border)]">
        ...
        <CardContent>
              ...
              {items.map((wf) => (
                <div key={wf.id} className="rounded-md border border-[var(--border)] p-4">
                  ...
                  <div className="mt-3 flex gap-2">
                    ...
                    <Link href={`/interactions/workflows/create`}><Button size="sm" variant="outline" className="border-[var(--border)] text-secondary-dark">Edit</Button></Link>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-4 flex gap-2">
            <Link href="/interactions/workflows/create"><Button className="button-primary">Create Workflow</Button></Link>
            <Link href="/interactions/workflows/approvals"><Button variant="outline" className="border-[var(--border)] text-secondary-dark">Go to Approvals</Button></Link>
          </div>
        </CardContent>
      </Card>
    </div>
