  ...
  return (
    <div className="space-y-6 px-4 md:px-0">{/* Mobile padding */}
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">{/* Stack on mobile */}
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-primary-dark">Create Workflow</h1>
          <p className="mt-1 text-sm md:text-base text-secondary-dark">Step-by-step wizard. Actions remain open for now. Scoping not included.</p>
        </div>
            ...
            {step !== 'Review' ? (
              ...
              <Button className="button-primary" disabled={saving} onClick={async () => {
                try {
                  ...
                  router.push('/interactions/workflows/active');
                } catch (e) {
                  console.error(e);
