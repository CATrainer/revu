...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(axiosError.response?.data?.detail || axiosError.response?.data?.message || 'Invalid email or password');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          <Link href="/" className="flex justify-center">
            <h1 className="text-3xl font-bold text-indigo-600 dark:text-indigo-400">Revu</h1>
          </Link>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="text-center">
            <Link href="/" className="text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300">
              Back to home
            </Link>
          </div>
        </form>
